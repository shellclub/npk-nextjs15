...
function InvoicesPage() {
  ...
  const header = (
    <div className="flex flex-auto flex-col py-4">
      <Typography sx={{ fontSize: '14px', color: '#94A3B8', mb: 0.5 }}>ใบแจ้งหนี้ {'>'} แสดงทั้งหมด</Typography>
      <div className="flex min-w-0 flex-auto flex-col gap-8 sm:flex-row sm:items-center">
        <div className="flex flex-auto items-center gap-8">
          <motion.span initial={{ x: -20 }} animate={{ x: 0, transition: { delay: 0.2 } }}>
            <Typography sx={{ fontSize: '28px', fontWeight: 800, color: '#1E293B' }}>ใบแจ้งหนี้</Typography>
          </motion.span>
                ...
                {data.map(inv => {
                  ...
                  return (
                    <TableRow key={inv.id} hover selected={selected.includes(inv.id)}
                      ...
                      <TableCell padding="checkbox" sx={{ pl: 2 }}><Checkbox checked={selected.includes(inv.id)} onChange={() => toggleSelect(inv.id)} size="small" /></TableCell>
