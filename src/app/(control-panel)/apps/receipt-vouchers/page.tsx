...
type ReceiptVoucher = {
  ...
  invoice?: {
    invoiceNumber: string; totalAmount: number;
    ...
    <Paper className="flex h-full w-full flex-auto flex-col overflow-hidden rounded-b-none" elevation={0}>
      ...
      : data.length === 0 ? (
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', py: 10 }}>
          ...
          <Typography sx={{ fontSize: '15px', color: '#94A3B8', mt: 1 }}>กดปุ่ม &quot;สร้างใหม่&quot; เพื่อบันทึกการรับเงิน</Typography>
        </Box>
      ) : (
        <>
            ...
            <Table stickyHeader>
              ...
              <TableBody>
                {data.map(rv => {
                  ...
                  return (
                    <TableRow key={rv.id} hover selected={selected.includes(rv.id)}
                      sx={{ cursor: 'pointer', '&:hover': { bgcolor: '#F0F9FF' }, '& td': { fontSize: '15px', color: '#334155', py: 1.5, borderBottom: '1px solid #F1F5F9' } }}>
                      <TableCell padding="checkbox" sx={{ pl: 2 }}><Checkbox checked={selected.includes(rv.id)} onChange={() => toggleSelect(rv.id)} size="small" /></TableCell>
                      <TableCell sx={{ whiteSpace: 'nowrap', fontWeight: 500 }}>{fmtDate(rv.date)}</TableCell>
                      <TableCell><Typography sx={{ fontSize: '15px', fontWeight: 600, color: '#0284C7' }}>{rv.voucherNumber}</Typography></TableCell>
                      <TableCell sx={{ fontWeight: 500 }}>{custName}</TableCell>
                      <TableCell>{rv.invoice?.invoiceNumber || '-'}</TableCell>
                      <TableCell align="center"><Chip label={paymentMethodLabels[rv.paymentMethod] || rv.paymentMethod} size="small" sx={{ bgcolor: '#E0F2FE', color: '#0284C7', fontWeight: 600, fontSize: '13px', borderRadius: '8px' }} /></TableCell>
                      <TableCell align="right" sx={{ fontWeight: 600, fontVariantNumeric: 'tabular-nums', fontSize: '16px !important' }}>{fmt(rv.amount)}</TableCell>
