...
type TaxInvoice = {
  ...
  invoice?: {
    ...
    workOrder?: {
      woNumber: string;
      quotation?: { customerGroup: { groupName: string; taxId?: string | null } } | null;
    } | null;
  } | null;
};
...
function TaxInvoicesPage() {
  ...
  const handlePrint = () => { if (menuItem) setSnackbar({ open: true, message: `กำลังพิมพ์ ${menuItem.taxInvoiceNumber}...`, severity: 'success' }); handleMenuClose(); };
            ...
            <Table stickyHeader>
              ...
              <TableBody>
                {data.map(ti => {
                  ...
                  return (
                    <TableRow key={ti.id} hover selected={selected.includes(ti.id)}
                      sx={{ cursor: 'pointer', '&:hover': { bgcolor: '#F0F9FF' }, '& td': { fontSize: '15px', color: '#334155', py: 1.5, borderBottom: '1px solid #F1F5F9' } }}>
                      <TableCell padding="checkbox" sx={{ pl: 2 }}><Checkbox checked={selected.includes(ti.id)} onChange={() => toggleSelect(ti.id)} size="small" /></TableCell>
                      ...
                      <TableCell sx={{ fontFamily: 'monospace', fontSize: '14px !important' }}>{custGroup?.taxId || '-'}</TableCell>
