  ...
  return (
    <div className="w-full p-24 md:p-32 lg:p-40">
      <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-16 mb-32">
        <div>
          <Typography sx={{ fontSize: '28px', fontWeight: 700, color: '#1E293B' }}>ใบกำกับภาษี</Typography>
          <Typography sx={{ fontSize: '16px', color: '#94A3B8', mt: 0.5 }}>ใบกำกับภาษี {'>'} แสดงทั้งหมด</Typography>
        </div>
        ...
        <Paper elevation={0} sx={{ borderRadius: '16px', border: '1px solid #E2E8F0', overflow: 'hidden' }}>
          ...
          : data.length === 0 ? (
            ...
            <TableContainer>
              <Table>
                ...
                <TableBody>
                  {data.map(ti => {
                    const sc = statusConfig[ti.status] || statusConfig['DRAFT'];
                    return (
                      <TableRow key={ti.id} hover selected={selected.includes(ti.id)} sx={{ cursor: 'pointer', '&:hover': { bgcolor: '#F0F9FF' }, '& td': { fontSize: '15px', color: '#334155', py: 2, borderBottom: '1px solid #F1F5F9' } }}>
                        <TableCell padding="checkbox" sx={{ pl: 3 }}><Checkbox checked={selected.includes(ti.id)} onChange={() => toggleSelect(ti.id)} size="small" /></TableCell>
                        <TableCell sx={{ whiteSpace: 'nowrap', fontWeight: 500 }}>{fmtDate(ti.date)}</TableCell>
                        <TableCell><Typography sx={{ fontSize: '15px', fontWeight: 600, color: '#0284C7' }}>{ti.taxInvoiceNumber}</Typography></TableCell>
                        <TableCell sx={{ fontWeight: 500 }}>{ti.customerGroup?.groupName || '-'}</TableCell>
                        <TableCell sx={{ fontFamily: 'monospace', fontSize: '14px' }}>{ti.customerGroup?.taxId || '-'}</TableCell>
                        <TableCell>{ti.invoice?.invoiceNumber || '-'}</TableCell>
                        <TableCell align="right" sx={{ fontWeight: 600, fontVariantNumeric: 'tabular-nums', fontSize: '16px !important' }}>{fmt(ti.totalAmount)}</TableCell>
          ...
          {!loading && data.length > 0 && (
            <Box sx={{ display: 'flex', justifyContent: 'space-between', px: 3, py: 2, borderTop: '1px solid #F1F5F9', bgcolor: '#FAFBFC' }}>
              ...
              <Typography sx={{ fontSize: '15px', fontWeight: 600, color: '#0284C7' }}>ยอดรวม <Box component="span" sx={{ fontSize: '17px', fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>{fmt(data.reduce((s, t) => s + Number(t.totalAmount), 0))}</Box> บาท</Typography>
            </Box>
          )}
        </Paper>
      </motion.div>
